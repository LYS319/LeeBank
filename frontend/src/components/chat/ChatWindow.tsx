...
export default function ChatWindow() {
    ...
    const { accountNo, memberId, openAuthModal } = useAuthStore();
    ...
    const sendText = async (text: string) => {
        ...
        try {
            const res = await chatApi.sendMessage(text.trim(), sessionId, accountNo ?? '');
            const data: ChatResponse = res.data;

            if (data.type === 'ELICITATION' && data.pendingAction) {
                // autoConfirm이 있으면 비밀번호 모달 없이 바로 실행
                if ((data.pendingAction as { autoConfirm?: boolean }).autoConfirm) {
                    try {
                        const confirmRes = await chatApi.confirm(sessionId, '', memberId ?? '', {
                            tool: data.pendingAction.tool,
                            params: { ...data.pendingAction.params, fromAccount: accountNo },
                            autoConfirm: true,
                        });
                        const card = parseResultCard(confirmRes.data.message);
                        addMessage('assistant', confirmRes.data.message, card ?? undefined);
                    } catch {
                        addMessage('assistant', '조회 중 문제가 생겼어요. 잠시 후 다시 시도해주세요.');
                    }
                } else {
                    // 이체 등 인증 필요한 도구 — 모달 표시
        ...
        <div className="chat">
            <header className="chat__header">
                <div className="chat__header-row">
                    <button className="chat__back" onClick={() => navigate('/home')} aria-label="홈으로">
            ...
            {messages.length <= 1 && (
                <div className="chat__suggestions">
                    {SUGGESTIONS.map((s) => (
                        <button key={s} className="chat__suggestion-chip" onClick={() => handleSuggestion(s)}>
                            {s}
                        </button>
                    ))}
                </div>

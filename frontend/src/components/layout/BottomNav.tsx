...
function TransferIcon({ active }: { active: boolean }) {
    return (
        <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
            <path
                d="M7 7h12M19 7l-3.5-3.5M19 7l-3.5 3.5"
                stroke="currentColor"
                strokeWidth="1.8"
                strokeLinecap="round"
                strokeLinejoin="round"
            />
            <path
                d="M17 17H5M5 17l3.5-3.5M5 17l3.5 3.5"
                stroke="currentColor"
                strokeWidth="1.8"
                strokeLinecap="round"
                strokeLinejoin="round"
...
export default function BottomNav() {
    ...
    return (
        <nav className="bottom-nav" aria-label="주요 메뉴">
            {/* 데스크탑 상단 네비에서만 보이는 로고. 클릭하면 로그인 후 메인인 /home으로 이동한다. */}
            <button className="bottom-nav__brand" onClick={() => navigate('/home')} aria-label="LeeBank 홈으로 이동">
                <span className="bottom-nav__brand-mark">B</span>
                <span className="bottom-nav__brand-name">LeeBank</span>
            </button>
